...
public class SheZhiFuKuanRenZhangHaoActivity extends Activity {
	private EditText fkzhEt = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.shezhifukuanzhanghao);
		
		fkzhEt = (EditText) this.findViewById(R.id.fk_account);
...
//				TranslateAnimation.ABSOLUTE, 0, TranslateAnimation.ABSOLUTE, 0);
	...
	public void close() {
		String account = fkzhEt.getText().toString();
		if(account.length() == 0) {
			Toast.makeText(this, "请输入正确的银行账号!", 0).show();
			return;
		}else {
			loginSp.setPayAccount(account);
