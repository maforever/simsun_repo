...
public class SheZhiShouKuanRenZhangHaoAcitivity extends Activity {
	private LoginSharedPreferenceService loginSp = null;
	private TextView sk_account = null;
	//效果
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.shezhishoukuanzhanghao);
		
		loginSp = new LoginSharedPreferenceService(this);
		sk_account = (TextView) this.findViewById(R.id.sk_account);
		initAnim();
...
//				TranslateAnimation.ABSOLUTE, 0, TranslateAnimation.ABSOLUTE, 0);
	...
	public void close() {
		String account = sk_account.getText().toString();
		if(account.length() == 0) {
			Toast.makeText(this, "请填写正确的银行账号!", 0).show();
			return;
		}else {
			loginSp.setNewPayeeAccount(account);
